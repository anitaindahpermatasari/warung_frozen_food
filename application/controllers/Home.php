...
class Home extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->post('submit', TRUE) == 'submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($get->num_rows() != 1) {
                    $this->session->set_flashdata('alert', 'Maaf User Tidak Dikenali. Silahkan Login Kembali');
                } else {
                    //fetch data
                    $u = $get->row();
                    //validasi password
                    if (password_verify($password, $u->password)) {
                        ...
                        $this->session->set_flashdata('alert', 'Maaf Password Salah. Silahkan Coba Lagi');
                    }
                }
            }
        }
    ...
    public function dashboard()
    {
        ...
        $data = [
            'title' => 'Website Warung Frozen Food'
        ];
        ...
        if ($this->input->post('submit', TRUE) == 'submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (!password_verify($this->input->post('password', TRUE), $f->password)) {
                    $this->session->set_flashdata('error', 'Maaf Password Salah. Silahkan Coba Lagi');
                    redirect('admin');
                }
                ...
                if ($up) {
                    $this->session->set_flashdata('success', 'Selamat Profil Berhasil Di Update');
    ...
    public function profil_pegawai()
    {
        ...
        if ($this->security->xss_clean($this->input->post('submit', TRUE)) == 'submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                //jika proses update berhasil, maka alihkan ke halaman data pegawai dan tampilkan pesan
                if ($up) {
                    $this->session->set_flashdata('success', 'Selamat Data Pegawai Berhasil Di Update');
                    //update session profil
                    $this->session->set_userdata(
                        array(
                            'User' => $this->security->xss_clean($this->input->post('fullname', TRUE)),
        ...
        if ($this->input->post('submit', TRUE) == 'submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (!password_verify($pwLama, $f->password)) {
                    $this->session->set_flashdata('error', 'Maaf Password Lama Salah. Silahkan Coba lagi.');
                    redirect('password');
                }
                //cek apakah password lama dan baru sama, jika sama maka munculkan pesan
                if ($pwBaru == $pwLama) {
                    $this->session->set_flashdata('error', 'Password Baru dan Password Lama Tidak Berubah');
                    redirect('password');
                }
                ...
                if ($update) {
                    $this->session->set_flashdata('success', 'Selamat Password Berhasil Di Update');
                } else {
                    $this->session->set_flashdata('error', 'Maaf Password Tidak Berhasil Di Update');
                }
